...
| `server.py` | Twenty-five MCP tools; `main()` entry point; `--http` flag switches stdio → HTTP/SSE; port auto-increments from 3749; `_cutlist_pipeline()` is the shared post-panel pipeline (per-thickness sheet optimisation, pricing, file output) behind both `generate_cutlist` and `generate_project_cutlist` |
...
Baseline: 288 scenarios / 983 assertions / 100% pass rate. Run the eval suite after any non-trivial change.
