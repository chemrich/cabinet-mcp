...
Baseline is 283 scenarios / 940 assertions; expect your additions to raise both counts. Remember evals run in **lite CI too** — assert on shapes available without CadQuery (e.g. the `ERROR:` result in lite for CadQuery-only tools), or gate accordingly.
